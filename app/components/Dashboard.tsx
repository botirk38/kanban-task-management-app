...
const Dashboard = () => {
  ...
  const statuses = Array.from(
    new Set(currentBoard?.columns.flatMap((column) => column.name))
  );
  const currentBoardWithIcons = useColumnIcons(
    currentBoard && currentBoard.columns ? currentBoard : boards[0]
  );
  ...
  const { openTask, handleStatusChange, toggleSubtaskCompleted } =
    useTaskOperations({
      currentBoard,
      setCurrentBoard,
      selectedTask,
      ...
      <main className="dark:bg-blue-gray min-h-screen overflow-y-hidden  z-0 lg:w-full lg:col-span-2 ">
        <section className=" overflow-y-visible grid place-items-start grid-cols-auto grid-rows-1 grid-flow-col p-4 w-[120rem] min-h-screen lg:w-[150rem] 2xl:min-w-[200rem] bg-blue-pale dark:bg-blue-dark z-0 lg:gap-10">
          <div className="flex flex-col justify-center  items-start  gap-4 w-full min-h-full">
            <button
              className=" w-80 text-purple-dark min-h-full font-bold lg:text-2xl hover:bg-purple-light hover:bg-opacity-20"
              onClick={() => setCreateBoardClicked((prevState) => !prevState)}
            >
              Create Board +
            </button>
          </div>
    ...
    <main className="dark:bg-blue-gray min-h-screen overflow-hidden z-0 lg:w-full lg:col-span-2 ">
      <div className=" hidden lg:flex lg:p-4 lg:justify-between lg:items-start lg:w-full lg:z-50 lg:sticky lg:left-0">
        ...
        <div className="flex gap-4">
          <ButtonPrimary onClick={handleAddTaskButtonClick}>
            Add new Task
          </ButtonPrimary>
          <button onClick={handleModalMenuClicked}>
            <Image
      ...
      {addTaskClicked && (
        <CreateTask
          onClose={() => setAddTaskClicked(false)}
          statuses={statuses}
        />
      )}
      ...
      {modalMenuClicked && (
        <ModalMenu
          actionType="Board"
          onClose={() => setModalMenuClicked(false)}
        />
      )}

      {addColumn && <EditBoard onClose={() => setAddColumn(false)} />}
